package com.djavid.dentistassistant.Geometry;
...
public class Geometry {
     ...
     *         <code>false</code> otherwise.
     */
    public static boolean lineSegmentTouchesOrCrossesLine(LineSegment first,
                                                          LineSegment second) {
        return isPointOnLine(first, second.first)
                || isPointOnLine(first, second.second)
                || (isPointRightOfLine(first, second.first) ^ isPointRightOfLine(first,
                second.second));
    }
    ...
    public static Point getLinesIntersection(LineSegment a, LineSegment b)
    {
        Point s1, s2;
        s1 = new Point(a.second.x - a.first.x, a.second.y - a.first.y);
        s2 = new Point(b.second.x - b.first.x, b.second.y - b.first.y);

        double s, t;
        s = (-s1.y * (a.first.x - b.first.x) + s1.x * (a.first.y - b.first.y)) / (-s2.x * s1.y + s1.x * s2.y);
        t = ( s2.x * (a.first.y - b.first.y) - s2.y * (a.first.x - b.first.x)) / (-s2.x * s1.y + s1.x * s2.y);

        if (s >= 0 && s <= 1 && t >= 0 && t <= 1) {
            return new Point(a.first.x + (t * s1.x), a.first.y + (t * s1.y));
        }
    ...
    public static double getAngleBetweenLines(LineSegment a, LineSegment b) {
        ...
        return Math.toDegrees(Math.acos(numerator / denumerator));
    }
